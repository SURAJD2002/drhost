...
function Products() {
  ...
  const addToCart = async (product, redirect = false) => {
    ...
    if (!session?.user) {
      toast.error(`Please log in to ${redirect ? 'proceed to checkout' : 'add items to cart'}.`, {
        duration: 4000,
        position: 'top-center',
        style: {
          background: '#ff4d4f',
    ...
    try {
      ...
      if (categoryError) throw categoryError;
      if (categoryData?.is_restricted && !location.state?.fromCategories) {
        toast.error(`Please select this category from the categories page to ${redirect ? 'proceed to checkout' : 'add products to cart'}.`, {
          duration: 4000,
          position: 'top-center',
          style: {
            background: '#ff4d4f',
