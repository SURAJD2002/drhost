...
function Account() {
  ...
  const handleCancelOrder = useCallback(
    async orderId => {
      ...
      try {
        const finalReason = cancelReason === 'Other (please specify)' ? cancelReason : cancelReason;
        const { error } = await supabase
          .from('orders')
          .update({ order_status: 'Cancelled', cancellation_reason: finalReason, updated_at: new Date().toISOString() })
          .eq('id', orderId);
        if (error) throw error;
        setOrders(prev =>
          prev.map(o =>
            o.id === orderId ? { ...o, order_status: 'Cancelled', cancellation_reason: finalReason } : o
          )
        );
        setCancelOrderId(null);
        setCancelReason('');
    ...
    <div className="td-account-container">
      ...
      <section className="td-account-section">
        ...
        {loading ? (
              ...
              {displayedOrders.map(order => {
                ...
                return (
                  <div key={order.id} className={`td-order-item ${order.isNew ? 'td-order-item-new' : ''}`}>
                    ...
                    <p>Total: ₹{(order.total || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</p>
                    <p>Status: {order.order_status}</p>
                    {order.order_status === 'Cancelled' && <p>Reason: {order.cancellation_reason}</p>}
                    <p>Ordered on: {formatDateTime(order.created_at)}</p>
                    {order.estimated_delivery && (
                      <p>Estimated Delivery: {formatDateTime(order.estimated_delivery)}</p>
                    )}
                    {order.payment_method === 'emi' && order.order_status === 'pending' && (
                      <p className="td-emi-pending">(Waiting for Approval)</p>
                    )}
                      ...
                      <div className="td-order-products">
                        ...
                        <div className="td-order-product">
                          <div className="td-order-product-details">
                            <p>
                              {order.emi_applications?.product_name || 'N/A'} - ₹
                              {(order.emi_applications?.product_price || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}
                            </p>
                            {profile?.is_seller && (
                              <>
                                <p>
                    ...
                    {cancelOrderId === order.id && (
                      <div className="td-cancel-modal" role="dialog" aria-labelledby={`cancel-modal-${order.id}`}>
                        ...
                        <select
                          ...
                          aria-label="Select cancellation reason"
                          className={cancelReason ? '' : 'td-input-error'}
                        >
                          <option value="">Select reason</option>
                          {(profile?.is_seller ? sellerCancelReasons : buyerCancelReasons).map(r => (
                            <option key={r} value={r}>
                        ...
                        {isCustomReason && (
                          <textarea
                            ...
                            placeholder="Custom reason"
                            aria-label="Custom cancellation reason"
                            className={`td-custom-reason-input ${cancelReason.trim() ? '' : 'td-input-error'}`}
                          />
                        )}
                        <div className="td-cancel-modal-buttons">
                          <button
                            ...
                            className="td-btn-confirm-cancel"
                            aria-label="Confirm order cancellation"
                            disabled={!cancelReason || (isCustomReason && !cancelReason.trim())}
                          >
                            Confirm
                          </button>
                          <button
          ...
          <p className="td-no-orders">{profile?.is_seller ? 'No orders on your products' : 'You have no orders yet.'}</p>
        )}
      </section>
