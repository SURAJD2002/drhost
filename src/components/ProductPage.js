...
// const TOAST_STYLES = {
//   error: {
//     background: '#1890ff', // Changed to blue
//     color: '#fff',
//     fontWeight: '500',
//     borderRadius: 'var(--border-radius)',
//     padding: 'calc(var(--spacing-unit) * 2)',
//   },
//   success: {
//     background: '#1890ff', // Changed to blue for consistency
//     color: '#fff',
//     fontWeight: '500',
//     borderRadius: 'var(--border-radius)',
//     padding: 'calc(var(--spacing-unit) * 2)',
...
//     productLoc.longitude === 0
//   ) {
//     console.warn('Invalid location data:', { userLoc, productLoc });
//     return null; // Return null instead of 0 to indicate invalid distance
//   }
//   const R = 6371; // Earth's radius in km
//   const dLat = ((productLoc.latitude - userLoc.lat) * Math.PI) / 180;
//   const dLon = ((productLoc.longitude - userLoc.lon) * Math.PI) / 180;
...
//     if (locationLoading || locationRetries >= maxLocationRetries) {
//       if (locationRetries >= maxLocationRetries) {
//         toast('Maximum location detection attempts reached. Using default location (Bengaluru).', {
//           duration: 6000,
//           position: 'top-center',
//           style: TOAST_STYLES.error,
//         });
...
//           setLocationLoading(false);
//           console.log('Using default location on retry (Bengaluru):', defaultLocation);
//           toast(`Location detection failed: ${error.message}. Using default location (Bengaluru).`, {
//             duration: 6000,
//             position: 'top-center',
//             style: TOAST_STYLES.error,
//           });
...
//       setLocationLoading(false);
//       console.log('No geolocation support, using default location (Bengaluru):', defaultLocation);
//       toast('Geolocation not supported. Using default location (Bengaluru).', {
//         duration: 6000,
//         position: 'top-center',
//         style: TOAST_STYLES.error,
//       });
...
//     } catch (err) {
//       console.error('Reviews fetch error:', err);
//       toast('Failed to load reviews. Please try again later.', {
//         duration: 4000,
//         position: 'top-center',
//         style: TOAST_STYLES.error,
//       });
...
//         console.warn(`Product ID ${id} has no category_id.`);
//         setError('Product category is missing.');
//         return;
//       }
...
//       if (distance !== null && distance > effectiveRadius) {
//         console.log(`Product ${productData.id} out of range: distance=${distance.toFixed(2)}km, radius=${effectiveRadius}km`);
//         toast(`Product is not available in your area (${distance.toFixed(2)}km > ${effectiveRadius}km).`, {
//           duration: 4000,
//           position: 'top-center',
//           style: TOAST_STYLES.error,
//         });
...
//       if (productData.categories?.is_restricted && !location.state?.fromCategories) {
//         toast('Please access this restricted category via the Categories page.', {
//           duration: 4000,
//           position: 'top-center',
//           style: TOAST_STYLES.error,
//         });
...
//       console.error('Product fetch error:', err);
//       setError(`Failed to load product: ${err.message}`);
//       toast(`Failed to load product: ${err.message}. Please try again.`, {
//         duration: 4000,
//         position: 'top-center',
//         style: TOAST_STYLES.error,
//       });
...
//         details: JSON.stringify({ productId: id, buyerLocation }),
...
//   const fetchRelatedProducts = useCallback(
//     async (product, retryCount = 0) => {
//       if (!product || !Number.isInteger(product.category_id) || !checkNetworkStatus()) {
//         console.warn('Skipping related products fetch:', {
//           hasProduct: !!product,
//           hasValidCategoryId: Number.isInteger(product.category_id),
//           networkStatus: navigator.onLine,
//         });
//         setRelatedProducts([]);
//         setIsRelatedLoading(false);
//         if (!Number.isInteger(product?.category_id)) {
//           toast('Product category is missing. Cannot load related products.', {
//             duration: 4000,
//             position: 'top-center',
//             style: TOAST_STYLES.error,
//           });
...
//           await supabase.from('debug_logs').insert({
//             log_type: 'error',
//             message: 'Related products fetch skipped: Invalid category_id',
//             details: JSON.stringify({ productId: product?.id, categoryId: product?.category_id, buyerLocation }),
//             created_at: new Date().toISOString(),
//           });
//         }
//         return;
...
//         setRelatedProducts([]);
//         setIsRelatedLoading(false);
//         toast('Location required to show related products. Please enable location or retry.', {
//           duration: 6000,
//           position: 'top-center',
//           style: TOAST_STYLES.error,
//         });
...
//           toast(`Found ${shuffled.length} related products!`, {
//             duration: 4000,
//             position: 'top-center',
//             style: TOAST_STYLES.success,
//           });
...
//         await supabase.from('debug_logs').insert({
//           log_type: 'error',
//           message: `Related products fetch failed: ${err.message}`,
//           details: JSON.stringify({ productId: product?.id, categoryId: product?.category_id, buyerLocation }),
//           created_at: new Date().toISOString(),
//         });
//         if (retryCount < 2) {
...
//         }
//         setRelatedProducts([]);
//         toast('Unable to load related products. Please try again later.', {
//           duration: 4000,
//           position: 'top-center',
//           style: TOAST_STYLES.error,
//         });
...
//     async (redirectToCart = false) => {
//       if (!product || isOutOfStock()) {
//         toast(isOutOfStock() ? 'This item is out of stock.' : 'Product not available.', {
//           duration: 4000,
//           position: 'top-center',
//           style: TOAST_STYLES.error,
//         });
...
//       if (variantId !== null && !Number.isInteger(variantId)) {
//         toast('Invalid variant selection.', {
//           duration: 4000,
//           position: 'top-center',
//           style: TOAST_STYLES.error,
//         });
...
//         setCart(updatedCart);
//         localStorage.setItem('cart', JSON.stringify(updatedCart));
//         toast(`${cartItem.title} added to cart!`, {
//           duration: 4000,
//           position: 'top-center',
//           style: TOAST_STYLES.success,
//         });
...
//           details: JSON.stringify({ productId: product.id, variantId, buyerLocation }),
//           created_at: new Date().toISOString(),
//         });
//       }
//     },
...
//   useEffect(() => {
//     if (product && Number.isInteger(product.category_id) && buyerLocation?.lat && buyerLocation?.lon && !locationLoading) {
//       fetchRelatedProducts(product);
//     }
//   }, [product, buyerLocation, locationLoading, fetchRelatedProducts]);
...
function ProductPage() {
  ...
  const getDisplayedImages = useMemo(
    () => () => {
      ...
      const productImages = Array.isArray(product?.images) ? product.images : [];
      const variantImages = Array.isArray(activeVariant?.images) ? activeVariant.images : [];
      return variantImages.length > 0 ? variantImages : productImages.length > 0 ? productImages : [DEFAULT_IMAGE];
    },
    [product, getActiveVariant],
  );
  ...
  const fetchProductReviews = useCallback(async (productId) => {
    try {
      ...
      try {
        await supabase.from('debug_logs').insert({
          message: `Reviews fetch failed: ${err.message}`,
          latitude: buyerLocation?.lat || 0,
          longitude: buyerLocation?.lon || 0,
          created_at: new Date().toISOString(),
        });
      } catch (logError) {
        console.error('Failed to log reviews error:', logError);
      }
      return [];
    }
  }, [buyerLocation]);
  ...
  const fetchProductAndVariants = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        console.error('Failed to log product fetch error:', logError);
      }
    } finally {
      setLoading(false);
    }
  ...
  const fetchRelatedProducts = useCallback(
    async (product, retryCount = 0) => {
      if (!product || !Number.isInteger(product.category_id) || !checkNetworkStatus()) {
        ...
        if (!Number.isInteger(product?.category_id)) {
          ...
          try {
            ...
            console.error('Failed to log related products error:', logError);
          }
        }
        return;
      }
      ...
      const cacheKey = `${product.id}-${product.category_id}-${Math.round(buyerLocation.lat * 1000) / 1000}-${Math.round(buyerLocation.lon * 1000) / 1000}`;
      if (relatedCache.current[cacheKey]) {
        console.log('Using cached related products:', cacheKey);
        setRelatedProducts(shuffleArray(relatedCache.current[cacheKey]));
        setIsRelatedLoading(false);
        return;
      }
      ...
      try {
        ...
        relatedCache.current[cacheKey] = shuffled;
        localStorage.setItem(CACHE_KEY, JSON.stringify(relatedCache.current));
        console.log('Fetched related products:', shuffled.length);
        setRelatedProducts(shuffled);
        if (shuffled.length > 0) {
          toast(`Found ${shuffled.length} related products!`, {
            duration: 4000,
  ...
  const addToCart = useCallback(
    async (redirectToCart = false, retryCount = 0) => {
      if (!product || isOutOfStock()) {
        toast(isOutOfStock() ? 'This item is out of stock.' : 'Product not available.', {
          duration: 4000,
          position: 'top-center',
      ...
      const cartItem = {
        ...
        seller_id: product.seller_id,
        seller_name: product.seller_name,
        variant_data: activeVariant ? activeVariant.attributes : null,
      };
      ...
      try {
        ...
        if (session) {
          ...
          if (existingCartItem) {
            ...
            console.log('Inserting cart item:', {
              ...
              seller_id: cartItem.seller_id,
              seller_name: cartItem.seller_name,
              variant_data: cartItem.variant_data,
            });
            const { data, error: insertError } = await supabase
              .from('cart')
              .insert({
        ...
        try {
          ...
          console.error('Failed to log cart addition:', logError);
        }
        ...
        console.error('Add to cart error:', err);
        if (err.code === 'PGRST204' || retryCount >= 2) {
          toast('Failed to add to cart due to a server configuration issue. Please try again later.', {
            duration: 4000,
            position: 'top-center',
            style: TOAST_STYLES.error,
          });
        ...
        console.log(`Retrying add to cart (attempt ${retryCount + 1})`);
        setTimeout(() => addToCart(redirectToCart, retryCount + 1), 1000);
      }
    },
    [product, cart, navigate, isRestricted, location.state, getActiveVariant, getDisplayedImages, isOutOfStock, buyerLocation],
  );
  ...
  useEffect(() => {
    if (product && !Number.isInteger(product.category_id)) {
          ...
          {JSON.stringify({
            ...
            offers: variants.length > 0
              ? variants.map((variant) => ({
                  ...
                  price: variant.price,
                  priceCurrency: 'INR',
                  availability: variant.stock > 0 ? 'http://schema.org/InStock' : 'http://schema.org/OutOfStock',
                  itemOffered: {
                    '@type': 'Product',
                    name: `${productName} (${Object.entries(variant.attributes || {})
                      .filter(([_, val]) => val && val.trim())
                      .map(([key, val]) => `${key}: ${val}`)
                      .join(', ')})`,
                    image: variant.images.length > 0 ? variant.images : displayedImages,
                  },
                }))
              : {
                  '@type': 'Offer',
