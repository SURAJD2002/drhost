...
//           <span
//             className="order-details__change-address"
//             onClick={() => toast('Address change not implemented yet. Contact support.', { icon: 'ℹ️' })}
//             aria-label="Change address"
//           >
//             CHANGE
//           </span>
...
//       const isBuyer = data.user_id === session.user.id;
//       const isOrderSeller = data.seller_id === session.user.id;
//       if (!isBuyer && !isOrderSeller) throw new Error('You are not authorized to view this order.');
...
//                   setCancelReason(e.target.value);
//                   setIsCustomReason(e.target.value === 'Other (please specify)');
//                   if (e.target.value !== 'Other (please specify)') setCustomReasonText('');
//                 }}
//                 aria-label="Select cancellation reason"
//                 className={cancelReason ? '' : 'order-details__input--error'}
//               >
...
function OrderDetails() {
  ...
  const fetchOrderDetails = useCallback(async () => {
    ...
    try {
      ...
      // Set product image (first image of first product or variant)
      ...
      if (reviewsData?.length) {
        const uniqueUserIds = Array.from(
          new Set([
            ...
            ...reviewsData.map(r => r.reviewed_id)
          ])
        );
        ...
        const mapName = id => profiles.find(p => p.id === id)?.name || 'Unknown User';

        setReviews(
          reviewsData.map(r => ({
            ...r,
            reviewer_name: mapName(r.reviewer_id),
            reviewed_name: mapName(r.reviewed_id),
          }))
        );
      } else {
        setReviews([]);
  ...
  // Date formatter with explicit IST timezone
  const formatDateTime = date => {
    if (!date) return 'N/A';
    return new Date(date).toLocaleString('en-IN', {
      day: '2-digit',
  ...
  // Timeline steps for order statuses
  const generateTimelineSteps = () => {
    if (!order) return [];
    const steps = [
      { label: 'Order Placed', date: formatDateTime(order.created_at), icon: '🧾' },
  ...
  const getCurrentStepIndex = () => {
    ...
    return map[order.order_status] ?? 0;
  };

  // Bubble left position for timeline status
  const getBubblePosition = () => {
    const timelineSteps = generateTimelineSteps();
    const currentStepIndex = getCurrentStepIndex();
    if (currentStepIndex === -1) return '0%';
  ...
  // Update order status handler (for seller)
  const handleUpdateStatus = async () => {
    if (!isSeller) return;
    if (!newStatus) {
      toast.error('Please select a new status.');
  ...
  return (
    <div className="order-details">
      ...
      <div className="order-details__info">
        ...
        <p>
          <strong>{order.order_status === 'delivered' ? 'Delivered on' : 'Estimated Delivery'}:</strong>{' '}
          {order.order_status === 'delivered' && order.actual_delivery_time ? formatDateTime(order.actual_delivery_time) : formatDateTime(order.estimated_delivery)}
        </p>

        {/* Ordered Items */}
        <div className="order-details__items">
          {order.order_items?.length > 0 ? order.order_items.map((item, index) => {
            const variant = item.variant_id && Array.isArray(item.product_variants) ? item.product_variants.find(v => v.id === item.variant_id) || null : null;
            const variantAttributes = variant?.attributes ? Object.entries(variant.attributes).filter(([k,v]) => v).map(([k,v]) => `${k}: ${v}`).join(', ') : null;
            return (
              <div key={index} className="order-details__item">
                {imageLoading ? <LoadingSpinner /> : (
                  <img
                    src={productImage}
                    alt={item.products?.title || `Product ${index + 1}`}
                    onError={e => e.target.src = 'https://arrettgksxgdajacsmbe.supabase.co/storage/v1/object/public/product-images/default.jpg'}
                    className="order-details__item-image"
                  />
                )}
                <div className="order-details__item-details">
  ...
  <div className="order-details__timeline-header">
    <span className="order-details__status-icon">📦</span>
    <span className="order-details__status-bubble" style={{ '--bubble-position': bubblePosition }}>
      <strong>Status:</strong> {order.order_status.charAt(0).toUpperCase() + order.order_status.slice(1)}
    </span>
    <span>Delivery by <strong>{timelineSteps[timelineSteps.length - 1]?.date || 'N/A'}</strong></span>
  </div>
  <div className="order-details__timeline-progress">
    {timelineSteps.map((step, index) => (
      <div
        key={step.label}
        className={`order-details__timeline-step ${
          index < currentStepIndex ? 'order-details__timeline-step--completed' :
          index === currentStepIndex ? 'order-details__timeline-step--current' :
          step.label === 'Cancelled' && order.order_status === 'cancelled' ? 'order-details__timeline-step--cancelled' : ''
        }`}
        aria-label={`${step.label}, ${index <= currentStepIndex ? 'completed' : index === currentStepIndex ? 'current' : 'pending'}`}
      >
        <div
          className={`order-details__timeline-dot ${
            index < currentStepIndex ? 'order-details__timeline-dot--completed' :
            index === currentStepIndex ? 'order-details__timeline-dot--current' :
            step.label === 'Cancelled' && order.order_status === 'cancelled' ? 'order-details__timeline-dot--cancelled' : ''
          }`}
        >
          {step.icon}
        </div>
        {index < timelineSteps.length - 1 && (
          <div
            className={`order-details__timeline-line ${
              index < currentStepIndex ? 'order-details__timeline-line--completed' :
              index === currentStepIndex ? 'order-details__timeline-line--current' : ''
            }`}
            style={{
              height: window.innerWidth < 640 ? `${index < currentStepIndex ? '100%' : index === currentStepIndex ? '50%' : '0%'}` : undefined,
              width: window.innerWidth >= 640 ? '100%' : undefined,
            }}
          />
        )}
        <div
          className={`order-details__timeline-label ${
            index < currentStepIndex ? 'order-details__timeline-label--completed' :
            index === currentStepIndex ? 'order-details__timeline-label--current' :
            step.label === 'Cancelled' && order.order_status === 'cancelled' ? 'order-details__timeline-label--cancelled' : ''
          }`}
        >
          <span>{step.label}</span>
          <span>{step.date}</span>
      ...
      {isSeller && order.seller_id === currentUserId && order.order_status !== 'cancelled' && order.order_status !== 'delivered' && (
        <div className="order-details__seller-actions">
          <h3>Update Order Status</h3>
          <select value={newStatus} onChange={e => setNewStatus(e.target.value)} className="order-details__status-select" disabled={actionLoading.updateStatus} aria-label="Select new order status">
            <option value="">Select Status</option>
          ...
          {newStatus.toLowerCase() === 'cancelled' && (
            <div className="order-details__cancel-reason">
              <h4>Cancellation Reason</h4>
              <select value={sellerCancelReason} onChange={e => { setSellerCancelReason(e.target.value); setIsCustomReason(e.target.value === 'Other (please specify)'); }} aria-label="Select cancellation reason">
                <option value="">Select Reason</option>
                {sellerCancelReasons.map(r => <option key={r} value={r}>{r}</option>)}
              </select>
              {isCustomReason && <textarea value={sellerCancelReason} onChange={e => setSellerCancelReason(e.target.value)} placeholder="Specify reason" className="order-details__custom-reason" aria-label="Custom cancellation reason" />}
            </div>
          )}
          <button onClick={handleUpdateStatus} disabled={actionLoading.updateStatus || !newStatus} className="order-details__button--update-status" aria-label="Update order status">
            {actionLoading.updateStatus ? 'Updating...' : 'Update Status'}
          </button>
        </div>
      )}
      ...
      {canCancel && (
        <div className="order-details__cancellation">
          <span>Cancellation available before shipping</span>
          <button className="order-details__cancel-button" onClick={() => setIsCancelling(true)} disabled={actionLoading.cancelOrder} aria-label="Cancel order">
            Cancel Order
          </button>
          {isCancelling && (
            <div className="order-details__cancel-modal" role="dialog" aria-labelledby="cancel-modal">
              <h3 id="cancel-modal">Cancel Order #{order.id}</h3>
              <select value={cancelReason} onChange={e => { setCancelReason(e.target.value); setIsCustomReason(e.target.value === 'Other (please specify)'); if (e.target.value !== 'Other (please specify)') setCustomReasonText(''); }} aria-label="Select cancellation reason" className={cancelReason ? '' : 'order-details__input--error'}>
                <option value="">Select reason</option>
                {buyerCancelReasons.map(r => <option key={r} value={r}>{r}</option>)}
              </select>
              {isCustomReason && <textarea value={customReasonText} onChange={e => setCustomReasonText(e.target.value)} placeholder="Specify reason" aria-label="Custom cancellation reason" className={`order-details__custom-reason ${customReasonText.trim() ? '' : 'order-details__input--error'}`} />}
              <div className="order-details__cancel-modal-actions">
                <button onClick={handleCancelOrder} className="order-details__button--confirm-cancel" disabled={actionLoading.cancelOrder || !validateCancelReason()} aria-label="Confirm order cancellation">{actionLoading.cancelOrder ? 'Cancelling...' : 'Confirm Cancel'}</button>
                <button onClick={() => { setIsCancelling(false); setCancelReason(''); setCustomReasonText(''); setIsCustomReason(false); }} className="order-details__button--close-cancel" aria-label="Close cancellation modal">Close</button>
              </div>
            </div>
          )}
        </div>
      ...
      <div className="order-details__reviews">
        ...
        {order.order_status === 'delivered' && (
          <div className="order-details__review-form">
            ...
            <textarea value={newReview.review_text} onChange={e => setNewReview({...newReview, review_text: e.target.value})} placeholder="Write your review..." className={newReview.review_text.trim() ? '' : 'order-details__input--error'} aria-label="Write your review" />
            <button onClick={handleSubmitReview} disabled={actionLoading.submitReview || !newReview.rating || !newReview.review_text.trim()} className="order-details__button--submit-review" aria-label="Submit review">{actionLoading.submitReview ? 'Submitting...' : 'Submit Review'}</button>
          </div>
        )}

        {reviews.length > 0 ? reviews.map(review => (
          <div key={review.id} className="order-details__review">
            <div className="order-details__review-header">
              <p><strong>{review.reviewer_name}</strong> reviewed <strong>{review.reviewed_name}</strong></p>
              <StarRating value={review.rating} disabled={true} />
            </div>
            <p className="order-details__review-text">{review.review_text}</p>
            <p className="order-details__review-date">Posted on {formatDateTime(review.created_at)}</p>
            {review.reply_text ? (
              <div className="order-details__review-reply"><p><strong>Reply:</strong> {review.reply_text}</p></div>
            ) : currentUserId === review.reviewed_id ? (
              <div className="order-details__reply-form">
                <textarea value={newReply} onChange={e => setNewReply(e.target.value)} placeholder="Write a reply..." className={newReply.trim() ? '' : 'order-details__input--error'} aria-label="Write a reply" />
                <button onClick={() => handleSubmitReply(review.id)} disabled={actionLoading.submitReply || !newReply.trim()} className="order-details__button--submit-reply" aria-label="Submit reply">{actionLoading.submitReply ? 'Submitting...' : 'Submit Reply'}</button>
              </div>
            ) : null}
          </div>
        )) : (
