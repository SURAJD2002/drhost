...
//   const fetchSellerData = useCallback(async (retryCount = 0, maxRetries = 3) => {
//     setLoading(true);
//     setError(null);
//     try {
//       const { data: { session }, error: sessionError } = await supabase.auth.getSession();
...
//       // Fetch products (exclude soft-deleted)
//       const { data: productsData, error: productsError } = await supabase
//         .from('products')
//         .select('id, title, price, images, stock')
//         .eq('seller_id', sellerId)
...
//       // Fetch product variants (filtered by non-deleted products)
//       const { data: variantsData, error: variantsError } = await supabase
//         .from('product_variants')
//         .select('id, product_id, attributes, price, stock, images')
//         .in('product_id', productsData.map(p => p.id))
...
//       // Fetch orders with correct foreign key
//       const { data: ordersData, error: ordersError } = await supabase
//         .from('orders')
//         .select(`
//           *,
...
//             quantity,
//             price,
//             products!order_items_product_id_fkey (title, images),
//             product_variants!order_items_variant_id_fkey (attributes, images)
//           )
//         `)
//         .eq('seller_id', sellerId)
//         .neq('payment_method', 'emi');
...
//       if (retryCount < maxRetries && err.code !== '42703' && err.code !== 'PGRST200') {
//         console.log(`Retrying fetchSellerData (${retryCount + 1}/${maxRetries})...`);
//         await new Promise(resolve => setTimeout(resolve, 1000 * (retryCount + 1)));
//         return fetchSellerData(retryCount + 1, maxRetries);
//       }
//       setError(`Error: ${err.message}`);
//       toast.error(`Error: ${err.message}`, {
//         duration: 4000,
...
//     const result = await Swal.fire({
//       title: 'Are you sure?',
//       text: 'This will hide the product from buyers. This action cannot be undone.',
//       icon: 'warning',
//       showCancelButton: true,
//       confirmButtonColor: '#d33',
//       cancelButtonColor: '#3085d6',
...
//     console.log(`Attempting to soft-delete product ID: ${productId}`);
//     try {
//       // Check for related order_items with non-cancelled orders
//       const { data: orderItems, error: orderItemsError } = await supabase
//         .from('order_items')
//         .select('id, order_id, orders!order_items_order_id_fkey(order_status)')
//         .eq('product_id', productId);
//       if (orderItemsError) throw orderItemsError;
//       const activeOrderItems = orderItems?.filter(item => item.orders?.order_status !== 'cancelled') || [];
//       if (activeOrderItems.length > 0) {
//         const orderIds = activeOrderItems.map(item => item.order_id).join(', ');
//         throw new Error(`This product has active orders (IDs: ${orderIds}). Please resolve or cancel these orders in the Orders section before deleting.`);
//       }
...
//       if (!productCheck || !productCheck.deleted_at) {
//         console.error('Product not soft-deleted:', productCheck);
...
//         console.log('Products after optimistic update:', newProducts);
//         return newProducts;
//       });
...
//         errorMessage = 'Database schema error: Could not update product variants. Please contact support.';
//       }
...
//         },
//       });
//       await fetchSellerData(); // Revert optimistic update
//     } finally {
//       setIsDeleting(false);
...
//               <ProductItem
//                 key={`product-${prod.id}`}
//                 product={{ ...prod, fullTitle: prod.title }}
//                 deleteProduct={deleteProduct}
//                 handleEditProduct={handleEditProduct}
//                 isDeleting={isDeleting}
//                 isDeletingThis={deletingProductId === prod.id}
//               />
//             ))}
//           </div>
//         )}
...
// const ProductItem = React.memo(({ product, deleteProduct, handleEditProduct, isDeleting, isDeletingThis }) => (
//   <div className={`seller-dashboard-product-item ${isDeletingThis ? 'deleting' : ''}`}>
//     <h3 className="seller-dashboard-product-title" data-full-text={product.fullTitle}>
//       {product.title}
//       {isDeletingThis && <span className="deleting-indicator">Deleting...</span>}
//     </h3>
//     <p className="seller-dashboard-text">Price: ₹{(product.price || 0).toLocaleString('en-IN')}</p>
//     <p className="seller-dashboard-text">Stock: {product.stock}</p>
//     {product.images?.length > 0 && (
...
//           return (
//             <div key={variant.id} className="seller-dashboard-variant-item">
//               <p className="seller-dashboard-text" data-full-text={attributesText}>
//                 {attributesText}
//               </p>
//               <p className="seller-dashboard-text">Price: ₹{(variant.price || 0).toLocaleString('en-IN')}</p>
//               <p className="seller-dashboard-text">Stock: {variant.stock}</p>
...
//         <h4 className="seller-dashboard-items-title">Items:</h4>
//         {order.order_items.map((item, idx) => {
//           const variantAttributes = item.product_variants?.attributes
...
//             : '';
//           return (
//             <div key={`${item.product_id}-${idx}`} className="seller-dashboard-text">
//               <p>
//                 {item.products?.title || 'Unnamed Product'}
//                 {variantAttributes && ` (${variantAttributes})`} - Qty: {item.quantity}
//               </p>
//               {item.product_variants?.images?.length > 0 && (
//                 <img
//                   src={item.product_variants.images[0]}
//                   alt="Variant"
//                   className="seller-dashboard-variant-image"
//                   onError={(e) => { e.target.src = 'https://dummyimage.com/80'; }}
//                 />
//               )}
//             </div>
...
function SellerDashboard() {
  ...
  const fetchSellerData = useCallback(async (retryCount = 0, maxRetries = 3) => {
    ...
    try {
      ...
      if (sellerData?.latitude != null && sellerData?.longitude != null) {
        ...
        const defaultLocation = { lat: 23.7957, lon: 86.4304 }; // Dhanbad, Jharkhand
        setSellerLocation(defaultLocation);
        await fetchAddress(defaultLocation.lat, defaultLocation.lon);
      }
  ...
  const fetchAddress = async (lat, lon) => {
    try {
      ...
      if (!response.ok) {
        throw new Error(`Mapbox failed: ${response.statusText}`);
      }
      const data = await response.json();
      if (data.features?.[0]?.place_name) {
        setAddress(data.features[0].place_name);
  ...
  return (
    <div className="seller-dashboard">
      ...
      <section className="seller-dashboard-section seller-info">
        ...
        <p className="seller-dashboard-text">Location: <span>{address}</span></p>
        <MapComponent
          initialLat={seller?.latitude || 23.7957}
          initialLon={seller?.longitude || 86.4304}
          onLocationSelect={handleMapLocationSelect}
        />
        <button
          onClick={handleDetectLocation}
      ...
      <img
        src={icon}
        alt="FreshCart Logo"
        className="seller-dashboard-icon"
      />
    </div>
  );
}
